...
public class ModBlockStateProvider extends BlockStateProvider {
    public ModBlockStateProvider(PackOutput gen, ExistingFileHelper exFileHelper) {
        super(gen, AE2Extras.MOD_ID, exFileHelper);
    }
    ...
    private void energyCell(EnergyCellBlock block) {
        ...
        ResourceLocation fillLevel = ResourceLocation.fromNamespaceAndPath( "ae2","fill_level");
        getVariantBuilder(block).forAllStates(state -> {
            int fullness = state.getValue(EnergyCellBlock.ENERGY_STORAGE);
            ModelFile modelFile = models().cubeAll(s+"_"+fullness,ResourceLocation.fromNamespaceAndPath("ae2","block/dense_energy_cell_"+fullness));
            return ConfiguredModel.builder().modelFile(modelFile).build();
        });
        ModelFile model = itemModels().getBuilder(s)
                        .parent(models().getExistingFile(modLoc("block/"+s+"_0")))
                .override().predicate(fillLevel,0.2f)
                .model(models().getExistingFile(modLoc("block/"+s+"_1"))).end()
                .override().predicate(ResourceLocation.fromNamespaceAndPath( "ae2","fill_level"),0.4f)
                .model(models().getExistingFile(modLoc("block/"+s+"_2"))).end()
                .override().predicate(fillLevel,0.6f)
                .model(models().getExistingFile(modLoc("block/"+s+"_3"))).end()
                .override().predicate(fillLevel,0.8f)
